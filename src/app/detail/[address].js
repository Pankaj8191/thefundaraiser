...
//       const storyData = await fetch(`https://ipfs.infura.io/ipfs/${Data.storyUrl}`)
//         .then(response => response.text())
//       const donationsData = await contract.queryFilter(contract.filters.Donated())
//       setStory(storyData)
...
//   function donate() {
//     // send transaction to donate funds to the campaign
//   }
...
//           <li key={donation.transactionHash}>
//             {donation.args.donor}: {donation.args.amount.toString()} wei
//           </li>
//         ))}
//       </ul>
//     </div>
...
//   const logs = await provider.getLogs({ fromBlock: 0, toBlock: 'latest', topics: campaignCreatedFilter.topics })
//   const campaigns = logs.map(log => ({ address: ethers.utils.getAddress(log.topics[1].slice(2)) }))
//   return {
//     paths: campaigns.map(campaign => ({ params: { address: campaign.address } })),
//     fallback: true
//   }
// }
